...
public class YoutubeDownService {
	...
	public int download(String youtubeUrl){
		...
		//생성된 자막이 없으면 파싱 시작	
		String flacCommand = "/win64/usr/bin/youtube-dl --extract-audio --audio-format flac -o \"%(id)s.%(ext)s\" "+url;
		
		String monoCommand = "ffmpeg -i "+url+".flac -ac 1"+" m"+url+".flac";
		String filename = "m"+url+".flac";
		try {
	...
	private String executeCommand(String flacCommand,String monoCommand) throws IOException, InterruptedException {
	    ...
	    new File("/tmp/test/"+url+".flac").delete();
	    url = "";
